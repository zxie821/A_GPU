...
"void print()                                \n"
"{                                          \n"
"    int index_i = get_global_id(0);          \n"
"    int index_j = get_global_id(1);           \n"
"    int local_i = get_local_id(0);     \n"
"    int local_j = get_local_id(1);     \n"
"    printf(\"Hello World!My threadId is global(%d, %d), local(%d, %d) \\n\", index_i, index_j,local_i, local_j);  \n"
"}                                          \n";
*/
const char* print_program =
"__kernel                                   \n"
...
"    printf(\"Hello World!My threadId is global(%d, %d, %d), local(%d, %d, %d) \\n\", i, j, z,local_i, local_j, local_z);  \n"
"}                                            \n";
...
int main(int argc, char *argv) {
  ...
  cl_program program = clCreateProgramWithSource(context, 1, (const char**)&print_program, NULL, &err); CHK_ERROR(err);
  ...
  cl_kernel kernel = clCreateKernel(program, "print", &err); CHK_ERROR(err);

    //size_t n_workitem[1] = { 256 };
    //size_t workgroup_size[1] = { 1 };
    //size_t n_workitem[] = { 16,16 };
    //size_t workgroup_size[] = { 16,16 };
