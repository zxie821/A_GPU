...
"   v->x += 0.01f * it;  v->y += 0.01f * it;  v->z += 0.01f * it;}; \n"
"__kernel                                                 \n"
...
float difference(const Particle* a, const Particle* b, const int size)
{
    float result = 0.0;
...
int main(int argc, char* argv) {
    ...
    auto start_time_gpu_datamove = std::chrono::high_resolution_clock::now();
    ...
    auto end_time_gpu_datamove = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> time_gpu_move = end_time_gpu_datamove - start_time_gpu_datamove;
    std::cout << "GPU memcpy time HtoD:" << time_gpu_move.count() << std::endl;
    ...
    cl_event event;
    // timing method from:https://stackoverflow.com/questions/23550912/measuring-execution-time-of-opencl-kernels
    start_time_gpu_datamove = std::chrono::high_resolution_clock::now();
    err = clEnqueueNDRangeKernel(cmd_queue, kernel, 1, NULL, n_workitem, workgroup_size, 0, NULL, &event); CHK_ERROR(err);
    clWaitForEvents(1, &event);
    end_time_gpu_datamove = std::chrono::high_resolution_clock::now();
    time_gpu_move = end_time_gpu_datamove - start_time_gpu_datamove;
    std::cout << "GPU kernel execution time:" << time_gpu_move.count() << std::endl;
    ...
    end_time_gpu_datamove = std::chrono::high_resolution_clock::now();
    time_gpu_move = end_time_gpu_datamove - start_time_gpu_datamove;
    std::cout << "GPU memcpy time DtoH:" << time_gpu_move.count() << std::endl;
