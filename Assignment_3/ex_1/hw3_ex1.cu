 ...
 */
double get_elapsed(clock_t t0, clock_t t1)
{
...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    int offset_out = irow * width;
    int offset = offset_out * 3;
    float *pixel = &image[offset + icol*3];
    image_out[offset_out + icol] = pixel[0] * 0.0722f + // B
                                    pixel[1] * 0.7152f + // G
                                    pixel[2] * 0.2126f;  // R
}
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    int index_x = block_offset_x + threadIdx.x;
    int index_y = block_offset_y + threadIdx.y;

    int idb = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;    
    __shared__ float sh_block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];
    if(index_x < width && index_y < height)
        sh_block[idb] = image[index_y*width + index_x];

    // fill extra two rows and cols
    if(threadIdx.x == 0 && threadIdx.y==0)
    {
        for(int i=BLOCK_SIZE_SH-2;i<BLOCK_SIZE_SH;++i)
        {
            for(int j=BLOCK_SIZE_SH-2; j<BLOCK_SIZE_SH;++j)
            {
                int image_w = block_offset_x+i;
                int image_h = block_offset_y+j;
                if(image_w < width - 1 && image_h < height-1)
                    sh_block[j*BLOCK_SIZE_SH + i] = image[image_h*width + image_w];
            }
        }
    }
    for(int i=BLOCK_SIZE_SH-2;i<BLOCK_SIZE_SH;++i)
    {
        int image_w = block_offset_x+i;
        int image_h = block_offset_y+threadIdx.y;
        if(image_w < width - 1 && image_h < height-1)
            sh_block[threadIdx.y*BLOCK_SIZE_SH + i] = image[image_h*width + image_w];
    }
    for(int j=BLOCK_SIZE_SH-2; j<BLOCK_SIZE_SH;++j)
    {
        int image_w = block_offset_x+threadIdx.x;
        int image_h = block_offset_y+j;
        if(image_w < width - 1 && image_h < height-1)
            sh_block[j*BLOCK_SIZE_SH + threadIdx.x] = image[image_h*width + image_w];
    }
    __syncthreads();
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        image_out[offset] = gpu_applyFilter(&sh_block[idb], BLOCK_SIZE_SH, gaussian, 3);
    }
}
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        float gx = gpu_applyFilter(&sh_block[idb],
                                    BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter(&sh_block[idb],
...
int main(int argc, char **argv)
{
    ...
    // Step 2: Apply a 3x3 Gaussian filter
    {
        ...
        elapsed[0] = get_elapsed(t[0], t[1]);
        int smemSize = (BLOCK_SIZE_SH*BLOCK_SIZE_SH)*sizeof(float);
        // Launch the GPU version
        t[0]= clock();
        gpu_gaussian<<<grid, block, smemSize>>>(bitmap.width, bitmap.height,
    ...
    // Step 3: Apply a Sobel filter
    {
        ...
        gpu_sobel<<<grid, block, smemSize>>>(bitmap.width, bitmap.height,
